		...
		=	function(_,hal_builder,uritemplate)
			...
			var	hal_collection_builder
				=	function(collection,options,type_collection)
				{
					...
					=	options.page||1
					,	ipp
					=	options.ipp||undefined
					,	url
					=	uritemplate(options.collection_url+'{?page,ipp,type}')
					,	last_page
					...
					= 	{
							...
							,	next
							=	{next:{href:config.url.expand({page:next_page,ipp:config.ipp,type:type_collection})}}
							,	prev
							=	{prev:{href:config.url.expand({page:prev_page,ipp:config.ipp,type:type_collection})}}
						...
						,	scrollable: function(hal,config)
							...
							=	1+parseInt(config.current_page)
							,	more
							=	{more:{href:config.url.expand({page:more_page,ipp:config.ipp,type:type_collection})}}
