			...
			=	function()
									...
									,	read: 	function(params,source)
												{
													...
													var get_store_function
													=	function(assoc_type)
														{
															var	type
															=	(_.isUndefined(assoc_type))
																?	(_.isString(params.subcollection))
																	?	'has-many'
																	:	'has-one'
																:	assoc_type
															,	type_functions
															...
															return	store[type_functions[type]]
															 	
														}	
													var store_function
													=	get_store_function(assoc_type)
				...
				=	_.objMap(
					...
					,	function(transform_spec,transform_entry)
						...
						return	function(promised_data,query,profile,collection_url)
							{
								var	isEmptyEmbedded
								=	function(embedded)
									{
				...
				var	GET
				=	function(url,item,parsed,source,profile)
					{	
						var	deferred
						=	Q.defer()
						,	collection_url
				...
				=	function(url,item,parsed)
					{
						...
						if	(_.isUndefined(parsed.subcollection) && _.isString(parsed.id))
							deferred
								.resolve(
									spec_builders[item](
