						...
						return	function(promised_data,collection_query,collection_url)
							{
								...
								.then(
									function(resolved_data)
									{
										deferred
											.resolve(
												_.isUndefined(resolved_data.items)
												?	async_build_msg(
														"error"
													,	"Resource not Found"
													,	404
													)
												:	_.isArray(resolved_data.items)
													?	async_build_many(resolved_data.items,resolved_data.count)
													:	async_build_one(resolved_data.items)
											)
									}
								)
				...
				=	function(method,url,body)
					{
						...
						if (_.isUndefined(spec[parts[1]]))
						{
							...
							if (!_.isUndefined(parts[3]) && _.isUndefined(store[spec[parts[1]].associations[parts[3]].target]))
							{
