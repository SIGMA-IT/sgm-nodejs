	...
	return	function(store,spec)
		{
			return {
				...
				check_rels: function(spec,spec_key)
				{
					_.each(
						spec.associations
					,	function(assoc,assoc_key)
						{
							if	(_.isUndefined(transforms[assoc.target]))
								console.log("error: no se encuentra transform: "+assoc.target+" | <<"+spec_key+":"+assoc_key+">>")
						}
					)
				}
					 		...
					 		,	function(assoc,assoc_key)
					 			{
									...
									// if	(old_embedded_value != "none" && old_embedded_value != "single")
									// 	console.log("Posible CR in spec: " +parent_spec.spec.storage.name+" assoc: "+tupla_obj.key + " embedded.type: "+old_embedded_value)
			...
			,	check_assocs: 	function(parent_spec,assoc_spec,level,visited)
					{
						...
						{
							_.isUndefined(assoc_spec.spec.associations)
								?visited
								:this.check_specs(
											assoc_spec.spec
										,	assoc_spec.key
