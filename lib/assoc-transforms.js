		...
		,	check_assoc_sintax: 	function(assoc,assoc_key)
				{
					...
					if(!attr_sintax)
					{
						console.log('error: wrong assoc-type <<'+assoc.type+'>> in ' +assoc_key+' : '+assoc.type)
		...
		//	table.foreign_key==foreign_table.id (donde, id es generico. Puede ser cualquier campo)
				{
					...
					{
						console.log('error assoc_rel: missing field <<'+assoc[attr_sintax.key]+'>> in mapping: '+assoc.source+':'+assoc_entry)
						throw 'error assoc_rel: missing field <<'+assoc[attr_sintax.key]+'>> in mapping: '+assoc.source+':'+assoc_entry
					}
