			...
			=	function(spec_key,assoc_key,item)
				{
					...
					=	spec[spec_key].associations[assoc_key]
					,	inverse
					=	assoc.type.indexOf("through") !== -1
						?	_.object(["to","from","rel","url"],[undefined])
						:	assoc_transforms.get_inverse(assoc,item)
								...
								=	function(assoc_key,item,merge)
									{
										...
										,	embedded_type
										=	(profile == "single" || checked)
											?	"none"
											:	associations[assoc_key].embedded.type

										if (checked)
										{
											console.log("Avoided Circular Reference in <<"+transform_entry+":"+assoc_key+">>")
											console.log("While trying to get "+resource_url)
										}
										else
