				...
				function(transform_spec,transform_entry)
				{
					...
					=	_.find(
						...
						,	function(assoc,assoc_key)
							{
								return	_.isString(assoc.parent)
							}
						)
					
					if	(!_.isUndefined(found))	{
						transform_spec.parent
						=	found.parent
						transform_spec.parent_key
						=	found.parent_key
					}
						...
						function(assoc,assoc_key)
						{
							...
							=	function(data)
								{									
									var	valid_template
									=	(assoc.type == "join")
										?	_.isString(assoc.through)
											?	assoc.through	+	assoc.key	+	assoc.through
											:	assoc.target	+	_.isString(assoc.key)
																	?	assoc.key
																	:	''
