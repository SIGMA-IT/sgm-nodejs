	...
	return	function(sources_list,loader)
		{
			...
			=	function(through,through_key,source,source_key)
				{
					var self
					=	this
					...
					return	_.filter(
							...
							,	function(through_item)
								{
									return through_item[through_key] == source[source_key]
								}
							)
				}
								...
								,	function(target_item)
									{
										return	_.find(
												...
												,	function(through_item)
													{
														return (target_item[filter.target_key] == through_item[filter.through_target_key]) && self.apply_filter(filter.query,target_item)
													}
												)
									}
								)
			...
			=	function(what,filter)
				{
					...
					return	_.isUndefined(filter.through)
							...
							:	_.find(
								...
								,	function(target_item)
									{
										return	target_item[filter.target_key] == through_item[filter.through_target_key]
									}
								)
				}
			this.find
