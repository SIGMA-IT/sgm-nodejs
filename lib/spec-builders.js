			...
			=	function(assoc,embedded_type,assoc_generic_link,item)
				{
					...
					.then(
						function(resolved_embeddeds)
						{
							deferred
								.resolve(
									self.isResourceNotFound(assoc.source,resolved_embeddeds)
									?	{links:{},embeddeds:{}}
									:	_.reduce(
											_.map(
											...
											,	function(item)
												{
													return	self.isResourceNotFound(assoc.source,resolved_embeddeds)
															?	{link:{},embedded:{}}
															:	_.object(
																	["link","embedded"]
																,	[
										...
										,	function(memo,resolved)
											{
												_.objMap(
													resolved.link
												,	function(value,key)
													{
														_.isArray(memo.links[key])
														?	memo.links[key].push(value)
														:	memo.links[key] = [value]
													}
												)
												if	(embedded_type != "none")
													...
													,	function(value,key)
														{
															_.isArray(memo.embeddeds[key])
															?	memo.embeddeds[key].push(value)
															:	memo.embeddeds[key] = [value]
														}
													)
												return memo
											}	
							...
							deferred
								.resolve(
									...
									:	_.reduce(
											...
											,	function(item)
												{
													return 	self.isResourceNotFound(assoc.source,{resource: item})
															?	{link:{},embedded:{}}
															: 	_.object(
																	["link","embedded"]
																,	[
										...
										deferred
											.resolve(
												self
													...
													,	Q(result)
													,	params.collection_query
													,	url.substring(0,url.length-parsed_url.query.length-(parsed_url.query.length ? 1 : 0))
													)
											)
									}
								)
